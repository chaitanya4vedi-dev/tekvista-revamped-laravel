    ...
    <div class="neo-card min-w-0 overflow-hidden p-6 sm:p-8">
        ...
        <form method="POST" action="{{ $isEdit ? route('blog.manage.update', $post->id) : route('blog.manage.store') }}" enctype="multipart/form-data" class="mt-6 grid min-w-0 gap-4">
            ...
            <label class="grid gap-2 text-sm font-bold text-[var(--text)]">Title
                <input type="text" name="title" value="{{ old('title', $isEdit ? $post->title : '') }}" required class="input-field" maxlength="180">
            </label>
            <label class="grid gap-2 text-sm font-bold text-[var(--text)]">SEO slug (optional)
                <input type="text" name="slug" value="{{ old('slug', $isEdit ? $post->slug : '') }}" class="input-field" maxlength="190" placeholder="enterprise-network-hardening-guide">
            </label>
            <label class="grid gap-2 text-sm font-bold text-[var(--text)]">Excerpt (optional, auto-generated if empty)
                <textarea name="excerpt" class="input-field" rows="3" maxlength="320">{{ old('excerpt', $isEdit ? $post->excerpt : '') }}</textarea>
            </label>
            ...
            <label class="grid gap-2 text-sm font-bold text-[var(--text)]">Meta description
                <textarea name="meta_description" class="input-field" rows="2" maxlength="320">{{ old('meta_description', $isEdit ? $post->meta_description : '') }}</textarea>
            </label>
