        ...
        @foreach ($odooServices as $slug => $service)
            <a href="{{ route('odoo.service', ['odooPage' => $slug]) }}" class="neo-card p-5 transition hover:border-[var(--accent)]">
                <div class="flex items-center gap-3">
                    ...
                    @else
                        <div class="grid size-12 shrink-0 place-items-center rounded-xl border border-[var(--line)] bg-[var(--surface-light)] text-[var(--accent)] sm:size-14"><i class="bi bi-grid-3x3-gap text-lg"></i></div>
                    @endif
